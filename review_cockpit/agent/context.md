...
`review_cockpit/frontend/src/views/` (one file per page) and
`review_cockpit/frontend/src/components/` (shared widgets reused across
...
store's validated accessor and prints the record as JSON:
    ...
    review_cockpit/agent/store-read pr <N> --section analysis   # just one section
    ...
    review_cockpit/agent/store-read issue <N> --section analysis  # just one section
    ...
    review_cockpit/agent/store-read activity recent --limit 50  # the recent action feed
...
confirmed chat write may have changed it without creating an activity row:
    ...
    review_cockpit/agent/store-read activity pr <N>      # landed actions on a PR
    review_cockpit/agent/store-read activity issue <N>   # landed actions on an issue
    review_cockpit/agent/store-read activity recent --limit 50   # the recent feed
...
- **Tooling problems → the meta-repo** `{feedback_repo}`, filed **as the operator**
  ...
  `enhancement` (it could be better):
      ...
      review_cockpit/agent/file-issue \
...
The `resubmit` helper owns the git mechanics; you author the edits in between:
    ...
    review_cockpit/agent/resubmit <pr> prepare
    ...
    review_cockpit/agent/resubmit <pr> diff
    ...
    review_cockpit/agent/resubmit <pr> push -m "<commit message>"
    ...
    review_cockpit/agent/resubmit <pr> abort    # discard the worktree, push nothing
...
ago*. To prove it still works on current code, merge the base branch into it:
    ...
    review_cockpit/agent/resubmit <pr> update
...
PR:
    ...
    review_cockpit/agent/reingest <pr>
...
not belong in a cluster, you can detach it locally — no full re-cluster needed:
    ...
    review_cockpit/agent/uncluster <pr> --from <cluster_id>   # one cluster
    review_cockpit/agent/uncluster <pr> --all                 # every cluster it's in
...
`gh api` isn't allowlisted — it's read-only only by default), use `gh-read`:
    ...
    review_cockpit/agent/gh-read file .gitattributes          # raw file on the default branch
    review_cockpit/agent/gh-read file Dockerfile --ref <sha>  # …at a branch/tag/SHA
    review_cockpit/agent/gh-read search 'eol=lf'              # code search, auto-scoped to the repo
...
Save it the moment it lands, with the *why* that makes it generalize:
    ...
    review_cockpit/agent/remember "<the learning>" --why "<why it generalizes>"
