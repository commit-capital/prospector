...
from review_cockpit.backend import agent_memory
...
from review_cockpit.backend import safety_guard
from review_cockpit.backend import subproc
...
_REMEMBER_ALLOW = ["Bash(review_cockpit/agent/remember:*)"]
...
_FILE_ISSUE_ALLOW = ["Bash(review_cockpit/agent/file-issue:*)"]
...
_UNCLUSTER_ALLOW = ["Bash(review_cockpit/agent/uncluster:*)"]
...
_GH_READ_ALLOW = ["Bash(review_cockpit/agent/gh-read:*)"]
...
_STORE_READ_ALLOW = ["Bash(review_cockpit/agent/store-read:*)"]
...
_REINGEST_ALLOW = ["Bash(review_cockpit/agent/reingest:*)"]
...
_RESUBMIT_ALLOW = ["Bash(review_cockpit/agent/resubmit:*)"]
...
def _issue_context(n: int) -> str:
    ...
    lines = [
        ...
        ("The pipeline's recorded disposition/rationale for this issue is "
         ...
         f"(`review_cockpit/agent/store-read issue {n} --section analysis`) "
