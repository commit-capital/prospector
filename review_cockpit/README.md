...
- **Writes as the operator, after confirmation:** on a machine that can mint the
  ...
  Separately, when `COCKPIT_FEEDBACK_REPO` is configured, `file-issue` may always
...
manual is `review_cockpit/agent/context.md`.
...
uv run uvicorn review_cockpit.backend.app:app --port 8787 --reload
# frontend
pnpm --dir review_cockpit/frontend install
...
uv run uvicorn review_cockpit.backend.app:app --port 8787   # open http://localhost:8787
```
