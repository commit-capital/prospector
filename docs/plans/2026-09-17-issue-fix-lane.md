...
## Roadmap — later steps, each planned in its own file when it starts

Lane: **L2** record shape, policy, queue (spec: Store, Lane state machine, Policy) · **L3** reproduce lane — `issue_triage/fix_lane.py`, `reproduce_issue.py`, `judge_repro.py`, `issue-sandbox-check`, `issue_fix_worker.py`, control/health/readiness wiring, `IssueFixPanel`, the "Post reproduction" comment (spec: Agent contracts, Sandbox primitives, Worker and control); run spike S12 first · **L4** fix → prove → review → park — `fix_issue.py`, `review_issue_fix.py`, `patch_hygiene.py`, `fix_patch_regate`, `fix_proof_bar` · **L5** blind trials against open PRs — `pipeline/evals/issue_fix_trials.py`, PR section `issue_repro`, `trials_view.py`, `lane_autonomy` (spec: Evaluation) · **L5b** history replay, the one-time calibration run on closed issues at the real pre-merge tree — `pipeline/patchkit.py` (`flatten`, `fresh_repo`), the lane core's `pre_patch` slot, `pipeline/evals/issue_fix_replay.py`, job `fix-replay` (spec: Evaluation, "History replay") · **L6** propose, built dry-run — `prospector_app/agent/propose`, `safety_guard` allowlist, scoped token, `executor.create_pr`, `fix_pr_body.py`, lane-authored-PR rules in `pipeline/gates.py`, hook, `propose_gc`, readiness; spikes S1–S6, S8–S11 before it goes live (spec: Propose path, The PR pipeline and lane-authored PRs) · **L7** hunter and budgets (needs F6) · **L8** unattended propose bar.

Funnel: **F2** threads · **F3** profile `issues` section, `issue_forms.py`, `grade_repro` v2 · **F4** `issue_gates.coverage` + rows, filters, chip · **F5** ASSESS · **F6** `fix_candidacy`, `issue_automation.classify`, Fix-funnel tab, Home cards · **F7** needs-info loop · **F8** FIX-MATCH · **F9** FIND-FIXED upgrades · **F10** `issue_sweep.py` and job (spec: Funnel, App surface).

Order constraints: F1→F4→F6; F2→F3, F5, F7; F6→F7, F8, F9→F10; L1→L2→L3→L4→L5→L6-live; L5 needs F1; L7 needs F6; the `issues` profile section lands with whichever of L2 / F3 comes first.
