...
def test_query_filters_severity(seeded):
    assert [r["ghsa_id"] for r in adv_mod.query_advisories(severity=["critical", "high"])["items"]] == [G1]
    assert [r["ghsa_id"] for r in adv_mod.query_advisories(severity="medium")["items"]] == [G2, G3]


def test_severity_sort_breaks_ties_oldest_first(seeded):
    out = adv_mod.query_advisories(sort="severity")
    assert [r["ghsa_id"] for r in out["items"]] == [G1, G3, G2]
    out = adv_mod.query_advisories(sort="severity", direction="asc")
    assert [r["ghsa_id"] for r in out["items"]] == [G2, G3, G1]
