...
test("selected advisories become chat subjects", () => {
  assert.deepEqual(
    subjectFromLocation("/security", "?advisory=GHSA-2222-3333-4444"),
    {
...
test("existing numbered subjects retain precise fields", () => {
  assert.deepEqual(subjectFromLocation("/prs/list", "?pr=42"), {
  ...
  assert.deepEqual(subjectFromLocation("/prs/clusters/7", ""), {
    kind: "cluster", key: "cluster:7", number: 7, label: "cluster 7",
  });
});

test("selected alerts become chat subjects", () => {
  const subject = subjectFromLocation("/security", "?alert_source=dependabot&alert=17");
...
test("invalid identifiers fall back to general context", () => {
  assert.equal(subjectFromLocation("/prs/list", "?pr=not-a-number").kind, "general");
  assert.equal(
    subjectFromLocation("/security", "?alert_source=unknown&alert=17").kind,
    "general",
  );
});
