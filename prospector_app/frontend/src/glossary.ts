...
const CLUSTER_STATE_GLOSSARY: Record<ClusterState, GlossaryEntry> = {
  ...
  "security-pending": {
    ...
    meaning: "A merge-routed PR here doesn't pass the merge gate yet — its security review is missing, stale (head moved or aged out), or non-GREEN; or dynamic verification hasn't confirmed the fix; or the PR is no longer clean (CI, conflicts, an automated reviewer or scanner).",
  },
  "ready": {
    title: "Ready",
...
export const TERMS: Record<string, GlossaryEntry> = {
  ...
  "col.review": { title: "Review", meaning: "Every automated code reviewer active on the repository (Greptile, CodeRabbit, …) and its verdict at the PR's head. Each must pass its own bar to merge; ⚠ marks a verdict from an earlier commit. Hover for each reviewer's reasoning." },
  "col.scans": { title: "Scans", meaning: "Every security scanner active on the repository (Superagent, Socket, …) and its verdict at the PR's head. An open finding or a failed scan blocks the merge gate under the scanner's own name." },
  ...
  "bulk.REVIEW_RETRIGGER": { title: "Re-trigger a review", meaning: "Post the reviewer's trigger mention on each selected PR so it reviews the current head again." },
  ...
  "review": { title: "Automated review", meaning: "The verdict of every code reviewer and security scanner active on the repository, each judged at the PR's current head. A PR is not clean while any of them objects or is still reviewing." },
