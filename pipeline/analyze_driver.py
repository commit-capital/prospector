...
Use `greptile_review`, not the bare score, to judge quality: a sub-5 member whose greptile_review is all nitpicks can still be the "merge" winner if it otherwise clears the bar above. When a member's greptile_review severity is "defects", prefer "request-changes" for it and lift the substantive findings verbatim into the asks.

You have a training cutoff. Do NOT reject, downgrade, or call a PR "invalid/unroutable/nonexistent" because an external identifier — a model id, API, package version, or release — looks unfamiliar. That is a post-cutoff fact you cannot verify from prior knowledge, and this repo tracks a fast-moving product. If a disposition hinges on whether such an identifier is real, confirm it with a read-only check (`gh`, docs/web) or route to "needs-human" (a person verifies) — never assert it is fake from memory.
...
- API contract breaks: renamed/removed response fields or changed status codes. Upstream's convention is consistent 400/401/403/404/409/422/500 responses; a contract break is at minimum a request-changes ask.
...
- Architecture policy: external adapters belong behind the adapter registry, never hardcoded into server/ or ui/; and upstream may close an uncoordinated feature PR regardless of code quality (pre-coordination happens in Discord #dev) — weigh that when the cluster outcome hinges on a feature nobody asked for.
...
Prefer the higher-greptile, tested, narrowest-correct implementation. When two competing PRs are otherwise close on correctness, tests, and Greptile, first prefer a `trusted` member (a trusted contributor named in the repository profile); this is a tiebreaker only — never pick a trusted PR over one that is clearly better on correctness, tests, or Greptile. If neither (or both) are trusted, break the tie on regression risk: prefer the one that merges cleanly (mergeable, drift not behind/conflicting) and is more recently maintained (updated_at) — a PR that needs rebasing or conflict-resolution carries more risk than one that applies as-is. Competing implementations: pick ONE winner, close the rest as dup of it; when the tie was close, state in that PR's rationale which factor broke it. When you prefer one PR over another because it does MORE (a superset), check that the extra changes are not already on __BRANCH__: start from each member's `already_on_master` count (redundant hunks are work that already landed upstream), and for anything it leaves unchecked read the current file with read-only `gh` (e.g. `gh api repos/__REPO__/contents/<path>`); never assume a PR's diff is net-new work. A change that already landed upstream is no edge: it adds nothing, and the stale re-add can silently regress a newer upstream change as __BRANCH__ moves — prefer the narrower PR. If you cannot confirm whether the extra work already landed, say so in the rationale rather than crediting the larger diff. Copy each member's head_sha from the bundle into your output.""".replace("__REPO__", settings.REPO).replace("__MERGE_BAR__", merge_bar_sentence())
