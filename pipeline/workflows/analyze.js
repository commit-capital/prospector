...
const ANALYSIS_SCHEMA = { type: 'object', properties: {
  ...
  prs: { type: 'array', items: { type: 'object', properties: {
    ...
    concerns: { type: 'array', description: 'for close-dup: the coverage map — one entry per substantive change in the PR', items: { type: 'object', properties: {
      label: { type: 'string', description: 'short name of the change' },
      paths: { type: 'array', items: { type: 'string' }, description: 'the files that carry the change' },
      coverage: { type: 'string', enum: ['landed', 'pr', 'unique'] },
      landed_sha: { type: 'string', description: 'for coverage landed: the default-branch commit carrying the change' },
      covered_by: { type: 'integer', description: 'for coverage pr: the covering PR number' },
      evidence: { type: 'string', description: 'one sentence naming where/how the change is covered' } },
