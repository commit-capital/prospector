...
The deterministic halves of each phase live in the drivers (`cluster_driver.py`,
later `analyze_driver.py` / `security_driver.py`): wave selection, validation,
store writes, stable IDs. The agentic halves run as Claude **Workflow** scripts
with schema-validated structured outputs — agents never write the store
directly; their output goes back through the driver.

## Phase 1 — CLUSTER (two stages)

```
# stage A: summaries (re-runnable; only PRs lacking a current summary)
uv run python cluster_driver.py fetch-diffs --max 200
uv run python cluster_driver.py write-batches --max 200   # pre-splits the wave + resets /tmp/pipeline-out
...
uv run python cluster_driver.py commit-summaries-dir       # commits every batch file on disk
...
uv run python cluster_driver.py write-cluster-units   # [--chunk N]: large N = one unit per subsystem
...
uv run python cluster_driver.py commit-clusters-dir   # commits every unit's proposals on disk
...
```
uv run python cluster_driver.py reset-stale-memberships  # detach PRs whose head moved since clustering
uv run python cluster_driver.py write-assign-units   # one unit per subsystem with new PRs
...
uv run python cluster_driver.py commit-assign-dir    # applies every unit's assignment on disk
uv run python views.py
```
...
```
uv run python cluster_driver.py write-straddle-units   # one+ unit per subsystem of clustered PRs
...
uv run python cluster_driver.py commit-assign-dir      # same additive commit path as assign
uv run python views.py
```

A gained membership reopens just that cluster (`outcome=None`) so ANALYZE re-runs
on it alone (Phase 3). Purely additive — a straddle pass never removes a PR from
...
```
uv run python threat_scan.py            # scan every PR with a cached diff
uv run python threat_scan.py --only 5174,5270   # rescan specific PRs
uv run python views.py
```
...
```
uv run python security_driver.py usage <PAST_WORKFLOW_LOG_DIR>
uv run python security_driver.py estimate --max 25 \
  --usage-log <PAST_WORKFLOW_LOG_DIR> \
  --require-calibration \
  --usage-percentile p90 \
  --verify-chunk-size 4 \
  --max-metered-tokens <TOKEN_LIMIT>
uv run python security_driver.py eligible --max 25 > /tmp/security-wave.json   # resets /tmp/security-out
...
uv run python security_driver.py commit-dir   # commits every verdict on disk
...
hardened image (`uv run python verify_driver.py build-image` — bakes the
profile's pnpm pin); sandbox/README.md carries the full machine checklist.

Pin and prepare the base image once per `main` (re-run on a re-pin):

```
uv run python verify_driver.py prepare-base --tier 1
#   pins main + the tier, clones it, scrubs + ASSERTS no secrets, builds
