...
`app/frontend/src/views/` (one file per page) and
`app/frontend/src/components/` (shared widgets reused across
...
store's validated accessor and prints the record as JSON:
    ...
    app/agent/store-read pr <N>                  # the whole PR record
    app/agent/store-read pr <N> --section analysis   # just one section
    app/agent/store-read cluster <CID>           # the whole cluster record
    app/agent/store-read issue <N>               # the whole issue record
    app/agent/store-read issue <N> --section analysis  # just one section
    app/agent/store-read threats                 # the threat registry
    app/agent/store-read activity pr <N>         # executed actions on one PR
    app/agent/store-read activity issue <N>      # executed actions on one issue
    app/agent/store-read activity recent --limit 50  # the recent action feed
...
confirmed chat write may have changed it without creating an activity row:
    ...
    app/agent/store-read activity pr <N>      # landed actions on a PR
    app/agent/store-read activity issue <N>   # landed actions on an issue
    app/agent/store-read activity recent --limit 50   # the recent feed
...
The `resubmit` helper owns the git mechanics; you author the edits in between:
    ...
    app/agent/resubmit <pr> push -m "<commit message>"
    ...
    app/agent/resubmit <pr> abort    # discard the worktree, push nothing
...
not belong in a cluster, you can detach it locally — no full re-cluster needed:
    ...
    app/agent/uncluster <pr> --from <cluster_id>   # one cluster
    app/agent/uncluster <pr> --all                 # every cluster it's in
...
`gh api` isn't allowlisted — it's read-only only by default), use `gh-read`:
    ...
    app/agent/gh-read file .gitattributes          # raw file on the default branch
    app/agent/gh-read file Dockerfile --ref <sha>  # …at a branch/tag/SHA
    app/agent/gh-read search 'eol=lf'              # code search, auto-scoped to the repo
...
Save it the moment it lands, with the *why* that makes it generalize:
    ...
    app/agent/remember "<the learning>" --why "<why it generalizes>"
